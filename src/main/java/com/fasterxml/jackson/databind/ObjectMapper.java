...
//    	_setupClassLoaderForDeserialization(valueType);
    ...
    public <T> T readValue(String content, Class<T> valueType)
        throws IOException, JsonParseException, JsonMappingException
    {
        System.out.println("readValue <Start>" + content);
     // !!! TODO
//    	_setupClassLoaderForDeserialization(valueType);
...
//    	_setupClassLoaderForDeserialization(valueType);
    ...
    protected Object _readMapAndClose(JsonParser jp, JavaType valueType)
        throws IOException, JsonParseException, JsonMappingException
    {
        System.out.println("_readMapAndClose" + valueType.toString());
        try {
            Object result;
            JsonToken t = _initForReading(jp);
            if (t == JsonToken.VALUE_NULL) {
                System.out.println("_readMapAndClose - NULL");
                // [JACKSON-643]: Ask JsonDeserializer what 'null value' to use:
                DeserializationContext ctxt = createDeserializationContext(jp,
                        getDeserializationConfig());
                result = _findRootDeserializer(ctxt, valueType).getNullValue();
            } else if (t == JsonToken.END_ARRAY || t == JsonToken.END_OBJECT) {
                System.out.println("_readMapAndClose - ARRAY / OBJECT");
                result = null;
            } else {
                System.out.println("_readMapAndClose - OTHERS");
                DeserializationConfig cfg = getDeserializationConfig();
                DeserializationContext ctxt = createDeserializationContext(jp, cfg);
                JsonDeserializer<Object> deser = _findRootDeserializer(ctxt, valueType);
                if (cfg.useRootWrapping()) {
                    System.out.println("_readMapAndClose - OTHERS - if");
                    result = _unwrapAndDeserialize(jp, ctxt, cfg, valueType, deser);
                } else {
                    System.out.println("_readMapAndClose - OTHERS - else");
                    result = deser.deserialize(jp, ctxt);
                    // この時点でsetterからの値も入っている
                }
                System.out.println("_readMapAndClose - OTHERS - " + result.getClass().getName());
                System.out.println(result.getClass().getName());
                if (result.getClass().getName().equals("com.fasterxml.jackson.dataformat.xml.deser.TestDeserialization$Mutable")) {
                    System.out.println("Mutable!!");
                    try {
                        java.util.ArrayList<Object> r = java.util.ArrayList.class.cast(result.getClass().getMethod("getMutableLabelsMethod", null).invoke(result, null));
